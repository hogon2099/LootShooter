...
namespace Invector
{
    ...
    public class vGameController : vMonoBehaviour
    {
        ...
        protected virtual void Start()
        {
			...
			//	vHUDController.instance.ShowText("Запуск уровня");
			FindPlayer();
        }
        ...
        protected virtual void OnCharacterDead(GameObject _gameObject)
        {
            ...
            {
                //if (displayInfoInFadeText && vHUDController.instance)
                //    vHUDController.instance.ShowText("Перезапуск уровня...");
        ...
        protected virtual IEnumerator RespawnRoutine()
        {
            ...
            if (playerPrefab != null && spawnPoint != null)
            {
                ...
                {
                    if (displayInfoInFadeText && vHUDController.instance)
                        vHUDController.instance.ShowText("Игрок уничтожен: " + oldPlayer.name.Replace("(Clone)", "").Replace("Instance", ""));
                    Destroy(oldPlayer);
                }
                ...
                if (displayInfoInFadeText && vHUDController.instance)
                {
                    vHUDController.instance.ShowText("Воскрешение игрока: " + currentPlayer.name.Replace("(Clone)", ""));
                }
        ...
        protected virtual void OnLevelFinishedLoading(Scene scene, LoadSceneMode mode)
        {
            ...
            {
                //if (displayInfoInFadeText && vHUDController.instance)
                //    vHUDController.instance.ShowText("Загрузка сцены: " + scene.name);
                return;
            }
            //if (displayInfoInFadeText && vHUDController.instance)
            //    vHUDController.instance.ShowText("Перезагрузка сцены");
            OnReloadGame.Invoke();
            FindPlayer();
        ...
        protected virtual void FindPlayer()
        {
            ...
            if (player)
            {
                ...
                //if (displayInfoInFadeText && vHUDController.instance)
                //    vHUDController.instance.ShowText("Агент инициализирован: " + currentPlayer.name.Replace("(Clone)", "").Replace("Instance", ""));
            }
